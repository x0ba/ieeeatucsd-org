...
export default function ReimbursementCreationPage({ onBack, onSubmitSuccess, initialData }: ReimbursementCreationPageProps) {
    ...
    useEffect(() => {
        if (user && !formData.additionalInfo && !initialData) {
            if (user.zelleInformation) {
                setFormData(prev => ({
                    ...prev,
                    additionalInfo: user.zelleInformation
                }));
            }
        }
    }, [user, initialData]);
    ...
    const handleReceiptUpload = async (receiptId: string, file: File) => {
        try {
            ...
            // Get URL from Convex (client-side)
            const downloadURL = (await fetch(`/_api/storage/${storageId}`, {
                method: 'GET',
            })).url;
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const submissionData = {
                ...
                department: formData.department as 'internal' | 'external' | 'projects' | 'events' | 'other',
