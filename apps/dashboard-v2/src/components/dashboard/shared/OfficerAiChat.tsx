  ...
  const renderLine = (line: string, i: number) => {
    ...
    if (line.startsWith("```")) {
      if (!inCodeBlock) {
        ...
        inCodeBlock = false;
        const rendered = (
          <CodeBlock key={`code-${i}`} className={`language-${codeLanguage}`}>
            {codeContent.join("\n")}
          </CodeBlock>
        );
        codeContent = [];
        ...
        <p key={`p-${i}`} className="mb-2">
          {line.split(/(`[^`]+`)/g).map((part, j) =>
            part.startsWith("`") && part.endsWith("`") ? (
              <CodeBlock key={`inline-${i}-${j}`} inline>
                {part.slice(1, -1)}
              </CodeBlock>
            ) : (
              part
...
export function OfficerAiChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Hello! I'm your AI assistant. Ask me detailed queries about events, reimbursements, or users.",
    },
  ]);
    ...
    try {
      const response = await fetch("/api/ai/query", {
        ...
        body: JSON.stringify({
          ...
          messages: messages
            .slice(-5)
            .map((m) => ({ role: m.role, content: m.content })), // Stripping steps for API
        }),
      });
      ...
      >
        ...
        <div className="flex items-center justify-between p-3 border-b bg-sidebar-accent/50">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="h-5 px-1.5 text-[10px] tracking-wider font-bold">
              BETA
            </Badge>
          </div>
        ...
        <div className="bg-blue-50/50 dark:bg-blue-900/10 px-4 py-2 border-b border-blue-100 dark:border-blue-900/20 flex gap-2 items-start">
          ...
          <p className="text-[11px] text-blue-700 dark:text-blue-300 leading-tight">
            This tool is in <span className="font-semibold">beta</span>. Please double check all info & report
            issues to the webmaster.
          </p>
        </div>
