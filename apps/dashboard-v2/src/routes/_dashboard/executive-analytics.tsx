...
const CHART_COLORS = ["#1d4ed8", "#0f766e", "#ca8a04", "#7c3aed", "#db2777", "#475569"];
...
function ExecutiveAnalyticsPage() {
          ...
          <Card className="bg-white border-slate-200">
            <CardHeader>
              ...
              <CardDescription className="text-slate-600">
                Executive analytics is available to Executive Officers and Administrators.
              </CardDescription>
            </CardHeader>
          </Card>
        </div>
  ...
  const statsCards = [
    ...
    },
    {
      ...
      icon: UserPlus,
      change: analytics.comparisons.newUsers,
    },
  ];
    ...
    <div className="w-full bg-slate-50 min-h-screen">
      <div className="mx-auto max-w-7xl p-4 md:p-6 space-y-5">
              ...
              >
                ...
                <SelectContent>
                  {analytics.fiscalYearOptions.map((option) => (
                    <SelectItem key={option.startYear} value={String(option.startYear)}>
                      {option.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            return (
              <Card key={card.key} className="bg-white border-slate-200">
                <CardHeader className="space-y-2 pb-2">
                  <CardDescription className="text-slate-500">{card.title}</CardDescription>
                ...
                <CardContent className="pt-0">
                  {card.change !== null ? (
                    ...
                    <p className="text-xs text-slate-500">Fiscal-year aggregate</p>
                  )}
                </CardContent>
              </Card>
            );
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={analytics.monthlyTrend} barGap={8}>
                    ...
                    />
                    <Bar dataKey="eventsHosted" name="Events Hosted" fill="#1d4ed8" radius={[6, 6, 0, 0]} />
                    <Bar dataKey="attendees" name="Attendees" fill="#0f766e" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
          ...
          <Card className="bg-white border-slate-200">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                {analytics.eventTypeBreakdown.map((entry, index) => (
                  <div key={entry.type} className="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2">
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          <Card className="xl:col-span-2 bg-white border-slate-200">
            <CardHeader>
              <CardTitle className="text-slate-900">Top Events by Attendance</CardTitle>
              <CardDescription className="text-slate-600">
          ...
          <Card className="bg-white border-slate-200">
            <CardHeader>
              <CardTitle className="text-slate-900">Engagement Quality</CardTitle>
              <CardDescription className="text-slate-600">
            ...
            <CardContent className="space-y-5">
              <div className="rounded-lg bg-slate-50 p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500">Average Attendance / Event</p>
              ...
              <div className="rounded-lg bg-slate-50 p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-xs uppercase tracking-wide text-slate-500">Attendee Coverage</p>
                  <p className="text-sm font-semibold text-slate-900">
                    {formatNumber.format(analytics.overview.attendeeCoverage)}%
                  </p>
                </div>
                <Progress
                  value={Math.min(Math.max(analytics.overview.attendeeCoverage, 0), 100)}
