...
import { Plus, Receipt, X, Upload, Loader2, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function ReceiptUploadStep({ receipts, setReceipts, errors, setErrors }: ReceiptUploadStepProps) {
    const [activeReceiptTab, setActiveReceiptTab] = useState(receipts.length > 0 ? receipts[0].id : '');
    ...
    const parseReceiptWithAI = async (receiptId: string, imageUrl: string, receiptFile?: { url: string; name: string; size: number; type: string }) => {
        try {
            ...
            if (!response.ok || !result.success) {
                const errorMessage = result.error || result.message || 'Failed to parse receipt';
            ...
            setParseResults(prev => ({
                ...
                [receiptId]: {
                    success: true,
                    message: 'Receipt parsed successfully! Review and edit the information below.'
                }
            }));
            ...
            let errorMessage = 'Failed to parse receipt. Please fill in the information manually.';
            if (error instanceof Error) {
                if (error.message.includes('AI service not configured')) {
                    errorMessage = 'AI service is not configured. Please contact support or enter data manually.';
