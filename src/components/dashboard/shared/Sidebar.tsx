...
export function Sidebar({ currentPath = "" }: SidebarComponentProps) {
      ...
      <div className="mb-6">
        ...
        <ul className="space-y-1">
          {[1, 2, 3, 4].map((itemIndex) => (
            <li key={itemIndex}>
              <div
                className={`flex items-center ${isCollapsed ? "justify-center px-3 py-3" : "px-3 py-2.5"}`}
              >
  ...
  return (
    <div
      className={`hidden lg:flex flex-col bg-white shadow-lg h-screen transition-all duration-300 ease-in-out ${isCollapsed ? "w-20" : "w-64"}`}
    >
      {/* Logo and Collapse Button - Fixed Header */}
      <div
        className={`flex-shrink-0 border-b border-gray-200 ${isCollapsed ? "p-4" : "p-5"}`}
      >
        {isCollapsed ? (
      ...
      {isLoading ? (
        ...
        <nav
          className={`flex-1 min-h-0 overflow-y-auto ${isCollapsed ? "px-2" : "px-3"} py-4`}
        >
                    ...
                    return (
                      <li key={index}>
                        <div className="relative group">
                          <a
                            href={item.href}
                            className={`flex w-full items-center text-sm font-medium rounded-lg transition-all duration-200 ${
                              isCollapsed
