...
import { generateEmailTemplate, createDetailRow, createInfoBox, IEEE_COLORS } from './templates/EmailTemplate';
...
export async function sendUserProfileUpdateEmail(
  ...
  try {
    ...
    if (data.changedByUserId) {
      const changedByDoc = await db.collection('users').doc(data.changedByUserId).get();
      if (changedByDoc.exists) {
        const changedByData = changedByDoc.data();
        changedByName = changedByData?.name || changedByData?.email || 'Unknown User';
      }
    }
...
export async function sendUserRoleChangeEmail(
  ...
  try {
    ...
    const roleHierarchy = ['Member', 'General Officer', 'Executive Officer', 'Administrator'];
    ...
    await resend.emails.send({
      ...
      to: [user.email],
      replyTo: replyToEmail,
      subject: isPromotion ? `Congratulations! You've Been Promoted to ${data.newRole}` : `Your Role Has Been Updated to ${data.newRole}`,
      html: emailHtml,
    });
